...
int input() {
  ...
  Event evt = handle_keypress(ch, st, fc);

  if (evt.type == EventType::MISC) {
    if (evt.desc == "idle") {
      return 0;
    }
    else if (evt.desc == "quit") {
      return -1;
    }
    else if (evt.desc == "qtmm") {
      wbkgdset(gamewin, COLOR_PAIR(Colors::win_brdr.cp));
      main_ms.highlight(0);
      st = AppState::MAIN_MENU;
      fc = FocusType::MENU;
    }
    else if (evt.desc == "other") {
      return 0; // Nothing here yet
    }
  }
  else if (evt.type == EventType::SELECT) {
    if (evt.desc == "confirm") {
  ...
  else if (evt.type == EventType::INTERACT) {
    if (evt.desc == "bed") {
      player.standing = !player.standing;
    }
  }
  else if (evt.type == EventType::CHAT) {
    if (evt.desc == "focus") {
      fc = FocusType::CHAT;
    }
    else if (evt.desc == "command") {
      fc = FocusType::CHAT;
      chat.i_am_typing("/");
    }
    else if (evt.desc == "unfocus") {
      fc = FocusType::GAME;
    }
    else if (evt.desc == "backspace") {
      chat.backspace();
    }
    else if (evt.desc == "send") {
      chat.send();
    }
    else if (evt.desc == "typing") {
      chat.i_am_typing_more(std::string(1, ch));
    }
  }
